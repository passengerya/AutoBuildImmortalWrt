...
> **派生子项目**：[AutoBuildImmortalTWrt](https://github.com/passengerya/AutoBuildImmortalTWrt) 以本项目 master 为蓝本，
> 采用**内嵌 store 方案**（第三方软件包直接由 [CloudRunFilesBuilder](https://github.com/passengerya/CloudRunFilesBuilder) 每日同步进仓库 `store/` 目录，
> 构建时挂载进容器使用，不再 clone 外部 store 仓库），并在本项目基础上吸收了构建稳定性修复。
...
https://github.com/passengerya/store/tree/master
